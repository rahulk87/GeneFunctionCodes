...
# Writing output per gene
#result <- results$gene
#write.csv(result,file="result.txt")

# Writing output per guide
reagent = results$reagent
result_reagents<-reagent[order(reagent$symbol, reagent$trcn_id), ]
write.csv(result_reagents,file="result_reagents.txt")
